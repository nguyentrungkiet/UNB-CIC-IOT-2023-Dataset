...
from tensorflow.keras.layers import Dense, Dropout, LSTM, Conv1D, MaxPooling1D, Input, Flatten
from tensorflow.keras.callbacks import EarlyStopping, ModelCheckpoint
from tensorflow.keras.optimizers import Adam

# Set output paths
input_file = 'd:\\SourceCode\\CICIoT2023_v4\\first_50000_rows.csv'
output_folder = 'D:\\SourceCode\\CICIoT2023_v4\\2_nhan\\cnn_lstm\\bieudo_ketqua'
...
# No need for feature selection with CNN-LSTM as it learns hierarchical features

# Reshape data for CNN-LSTM (samples, timesteps, features)
# We'll divide the features into multiple timesteps to make pooling work
print("\nReshaping data for CNN-LSTM...")
n_features = X_train_scaled.shape[1]
# Define the number of timesteps (aim for at least 4 to allow pooling)
n_timesteps = 4  # We'll create 4 timesteps
n_features_per_timestep = n_features // n_timesteps  # Features per timestep

# If features aren't perfectly divisible, we'll truncate slightly
effective_features = n_features_per_timestep * n_timesteps
print(f"Using {n_timesteps} timesteps with {n_features_per_timestep} features each (total: {effective_features} features)")
...
model = Sequential([
    ...
    MaxPooling1D(pool_size=2),  # Now should work with multiple timesteps
    Dropout(0.2),
...
model_checkpoint = ModelCheckpoint(os.path.join(output_folder, 'best_cnn_lstm_model.keras'), 
...
# Format probabilities for log_loss calculation
...
# Generate learning curves from history for CNN-LSTM
...
# 3. Feature importance visualization - CNN-LSTM doesn't provide direct feature importance
# We can use permutation importance or other methods if needed
print("Feature importance is not directly available for CNN-LSTM models.")

# 4. Learning Curves for CNN-LSTM
