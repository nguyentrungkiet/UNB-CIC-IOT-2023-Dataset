...
from sklearn.inspection import permutation_importance  # For feature importance calculation

# Set output paths
input_file = 'd:\\SourceCode\\CICIoT2023_v4\\first_50000_rows.csv'
output_folder = 'D:\\SourceCode\\CICIoT2023_v4\\2_nhan\\SVM\\bieudo_ketqua'  # Changed output folder
...
# Initialize SVM classifier (replaced RandomForest with SVM)
model = SVC(
    ...
    random_state=42,        # For reproducibility (same as RandomForest)
    verbose=True,           # Show progress
    cache_size=1000         # Memory cache size in MB
)
...
# For SVM, we'll generate learning curve based on different C parameter values
# instead of number of trees
print("\nGenerating learning curve data for SVM...")

# Define C parameters for learning curve
...
print("Calculating metrics for different C values...")
for c_value in c_values:
    # Create a model with current C value
    temp_model = SVC(
        C=c_value,
        kernel='rbf',
        gamma='scale',
...
# (SVMs don't have built-in feature importance like tree-based models)
...
# Create learning curves - adjusted for SVM
print("\nCreating learning curves...")

# Plot both accuracy and loss learning curves
plt.figure(figsize=(16, 7))

# Plot accuracy learning curves with C values on log scale
plt.subplot(1, 2, 1)
plt.semilogx(c_values, train_metrics['accuracy'], 'bo-', label='Training Accuracy')
plt.semilogx(c_values, val_metrics['accuracy'], 'ro-', label='Validation Accuracy')
plt.grid(True)
plt.title('SVM Learning Curves - Accuracy', fontsize=16)
plt.xlabel('C Parameter (log scale)', fontsize=12)
plt.ylabel('Accuracy', fontsize=12)
plt.legend(loc='lower right', fontsize=10)
plt.ylim([0, 1.0])

# Plot loss learning curves with C values on log scale
plt.subplot(1, 2, 2)
plt.semilogx(c_values, train_metrics['loss'], 'b-', label='Training Loss')
plt.semilogx(c_values, val_metrics['loss'], 'r-', label='Validation Loss')
plt.grid(True)
plt.title('SVM Learning Curves - Loss', fontsize=16)
plt.xlabel('C Parameter (log scale)', fontsize=12)
...
# Create a simpler loss curve
plt.figure(figsize=(10, 6))
plt.semilogx(c_values, train_metrics['loss'], 'b-', label='Train')
plt.semilogx(c_values, val_metrics['loss'], 'r-', label='Validation')
plt.xlabel('C Parameter (log scale)')
plt.ylabel('Log Loss')
plt.title('SVM Log Loss')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.savefig(os.path.join(output_folder, 'svm_learning_curve.png'))
plt.close()
...
try:
    ...
    # For SVM, shap_values might be a list with one array per class
