...
import os
import time
# Import TensorFlow instead of sklearn models
...
output_folder = 'D:\\SourceCode\\CICIoT2023_v4\\4_nhan\\cnn_lstm\\bieudo_ketqua'  # Updated output folder path
...
# Instead of feature selection, we'll reshape the data for CNN-LSTM
print("\nReshaping data for CNN-LSTM...")

# For CNN-LSTM, we need a 3D input: (samples, time steps, features)
# Let's create time steps by splitting the features
n_features = X_train_scaled.shape[1]
# Number of time steps for the CNN-LSTM model
n_timesteps = 4  # Using 4 time steps to enable pooling
...
# Reshape to 3D format: (samples, time steps, features)
...
# Selected feature names will be all features organized by time steps
selected_feature_names = X.columns[:effective_features].tolist()

# Convert target to categorical for CNN-LSTM
...
print(f"CNN-LSTM input shape: {X_train_reshaped.shape}")
...
model = Sequential([
    # CNN part
    Input(shape=(n_timesteps, n_features_per_timestep)),
    ...
    Dense(units=len(label_mapping), activation='softmax')  # Multi-class output
])
...
# 3. Learning Curves for CNN-LSTM (from history)
