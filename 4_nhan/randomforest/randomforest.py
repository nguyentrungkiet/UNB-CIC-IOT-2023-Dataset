...
# Removed feature selection import since RandomForest has built-in feature selection

# Set output paths
input_file = 'd:\\SourceCode\\CICIoT2023_v4\\first_50000_rows.csv'
output_folder = 'D:\\SourceCode\\CICIoT2023_v4\\4_nhan\\randomforest\\bieudo_ketqua'  # Updated path
...
# We'll use all features since Random Forest has built-in feature selection
...
# Get feature importance directly from Random Forest
...
# Try to use SHAP values for Random Forest if available
try:
    import shap
    explainer = shap.TreeExplainer(model)
    shap_values = explainer.shap_values(X_test_selected)
    
    # For Random Forest with multiple classes, shap_values might be a list with one array per class
    plt.figure(figsize=(12, 10))
    if isinstance(shap_values, list):  # Multi-class case
        # Plot SHAP values for a specific class (e.g., the first class)
        class_to_plot = 0  # Can be changed to focus on a different class
        class_name = label_mapping[class_to_plot]
        shap.summary_plot(shap_values[class_to_plot], X_test_selected, 
                        feature_names=X.columns, show=False,
                        title=f'SHAP Values for {class_name}')
        plt.title(f'SHAP Feature Importance for {class_name}', fontsize=16)
    else:  # Binary case or single output
