...
from sklearn.feature_selection import SelectFromModel  # Added missing import

# Set output paths
input_file = 'd:\\SourceCode\\CICIoT2023_v4\\first_50000_rows.csv'
output_folder = 'D:\\SourceCode\\CICIoT2023_v4\\4_nhan\\SVM\\bieudo_ketqua'  # Updated output folder path
...
# SVM doesn't need explicit feature selection for most cases, but we'll keep it for consistency
print("\nPerforming feature selection...")
# We'll use the scikit-learn SelectFromModel but with a simpler model
from sklearn.linear_model import SGDClassifier
selector = SelectFromModel(SGDClassifier(penalty='l1', alpha=0.001, random_state=42, loss='log_loss'))
...
# Initialize SVM with parameters for multi-class classification
model = SVC(
...
# Generate learning curves for SVM with different C values
print("\nGenerating learning curves for SVM...")
...
for c in c_values:
    ...
    # Calculate scores
    train_c_scores.append(accuracy_score(y_train, train_pred))
    val_c_scores.append(accuracy_score(y_val, val_pred))

# Save learning curve data based on C values
c_learning_curve_df = pd.DataFrame({
    'C_Value': c_values,
    'Train_Accuracy': train_c_scores,
    'Val_Accuracy': val_c_scores
})
c_learning_curve_df.to_csv(os.path.join(output_folder, 'c_learning_curve_data.csv'), index=False)

# Also generate standard learning curve
train_sizes, train_scores, val_scores = learning_curve(
    ...
    train_sizes=np.linspace(0.1, 1.0, 5), n_jobs=-1  # Reduced number of points for SVM since it's slower
)
...
# 1. Confusion Matrix Heatmap - adjusted to avoid overlapping text
plt.figure(figsize=(16, 12))  # Increased figure size to provide more space
sns.heatmap(cm, annot=True, fmt='d', cmap='Blues',
            ...
            annot_kws={"size": 14})  # Adjusted annotation size for better readability
...
plt.xticks(fontsize=12, rotation=45, ha='right')  # Rotated x-axis labels to avoid overlap
...
svm_importances.to_csv(os.path.join(output_folder, 'feature_importance.csv'), index=False)

# Plot top features
plt.figure(figsize=(14, 10))
n_features = min(20, len(selected_feature_names))
sns.barplot(x='Importance', y='Feature', data=svm_importances.head(n_features))
...
# Plot C-based learning curve for SVM
plt.figure(figsize=(12, 8))
plt.semilogx(c_values, train_c_scores, 'o-', color='b', label='Training Accuracy')
plt.semilogx(c_values, val_c_scores, 'o-', color='r', label='Validation Accuracy')
plt.title('SVM Model Performance vs. C Parameter', fontsize=16)
plt.xlabel('C Parameter (log scale)', fontsize=14)
plt.ylabel('Accuracy', fontsize=14)
plt.grid(True)
plt.legend(loc='best', fontsize=12)
plt.tight_layout()
plt.savefig(os.path.join(output_folder, 'c_learning_curves.png'))
plt.close()
...
plt.title('Learning Curves - SVM', fontsize=16)
