...
output_folder = 'D:\\SourceCode\\CICIoT2023_v4\\8_nhan\\mamba\\bieudo_ketqua'
...
print(f"Using all {len(selected_feature_names)} features with Mamba")
...
x = Dense(units=len(label_mapping), activation='softmax')(x[:, 0, :])  # Use first timestep for classification
...
# Remove the problematic learning_curve code and replace it
print("\nGenerating learning curves...")
# Instead of using sklearn learning_curve which requires XGBoost, 
# we'll just use the history data from our Keras model

# Using the already-generated history data instead of learning_curve function
...
plt.figure(figsize=(18, 15))  # Keep the same size as original
sns.heatmap(cm, annot=True, annot_kws={"size": 60}, fmt='d', cmap='Blues',
            xticklabels=[label_mapping[i] for i in range(len(label_mapping))],
            yticklabels=[label_mapping[i] for i in range(len(label_mapping))])
plt.title('Mamba Model: Confusion Matrix', fontsize=32)
...
# 3. Feature Importance - approximate for Mamba using permutation importance
try:
    feature_impacts = []
    ...
    # Test the impact of perturbing each feature (sample 20 features if too many)
    feature_indices = np.random.choice(
        X_test_scaled.shape[1], 
        size=min(20, X_test_scaled.shape[1]), 
        replace=False
    ) if X_test_scaled.shape[1] > 20 else range(X_test_scaled.shape[1])
    ...
    for i in feature_indices:
        ...
        feature_impacts.append((X.columns[i], impact))
    
    # Create feature importance dataframe
    feature_importance = pd.DataFrame(feature_impacts, columns=['Feature', 'Importance'])
    ...
    plt.title('Mamba Model: Feature Importance', fontsize=16)
    plt.xlabel('Importance (Accuracy Drop when Feature Permuted)', fontsize=14)
...
# 4. Learning Curves from Mamba training history
...
plt.tight_layout()
plt.savefig(os.path.join(output_folder, 'mamba_learning_curve.png'))
plt.close()
