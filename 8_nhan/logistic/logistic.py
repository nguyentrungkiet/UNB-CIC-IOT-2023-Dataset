...
output_folder = 'D:\\SourceCode\\CICIoT2023_v4\\8_nhan\\logistic\\bieudo_ketqua'  # Updated output folder path
...
# 3. Feature Importance for each class - modified to handle 8 classes
# Create a larger figure since we have 8 classes
plt.figure(figsize=(20, 16))
n_features = min(10, len(selected_feature_names))  # Reduced to 10 top features to save space

# Create a 3x3 grid for subplots (9 slots for 8 classes)
n_rows, n_cols = 3, 3
subplot_count = 1
...
for i, class_name in label_mapping.items():
    ...
    plt.subplot(n_rows, n_cols, subplot_count)
    sns.barplot(x='Importance', y='Feature', data=feature_importance.head(n_features))
    plt.title(f'Top Features: {class_name}', fontsize=12)
...
# Save feature importance for each class - no change needed here
for i, class_name in label_mapping.items():
    if i == len(label_mapping) - 1 and len(label_mapping) > 2:
        continue
